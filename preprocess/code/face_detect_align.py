...
if __name__ == "__main__":
    ...
    f = open('./result/img_list.txt', 'w')

    for img_id in os.listdir('./data/CASIA_WebFace/'):
        for file in os.listdir('./data/CASIA_WebFace/' + img_id):
            img = cv2.imread('./data/CASIA_WebFace/' + img_id + '/' + file)
            print('deal with ' + img_id + '/' + file)
            detect_info = DetImage(pnet, rnet, onet, img, False)
            face_index = GetFaceIndex(detect_info, img)
            if face_index == -1:
                f.write('./data/CASIA_WebFace/' + img_id + '/' + file + '\n')
                continue
            else:
                if face_alignment == True:
                    ...
                    face_input = img
                if not os.path.exists('../result/CASIA_WebFace/' + img_id):
                    os.mkdir('../result/CASIA_WebFace/' + img_id)
                # face_gray = cv2.cvtColor(face_input, cv2.COLOR_BGR2GRAY)
                face_resized = cv2.resize(face_input,(112, 96))
                cv2.imwrite('../result/CASIA_WebFace/' + img_id + '/' + file, face_resized)
