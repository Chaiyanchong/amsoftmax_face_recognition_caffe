...
# img_save_dir = './result/CASIA_WebFace' + '-' + str(img_resize[0]) + 'x' + str(img_resize[1])
root_dir = '/home/chujie/PycharmProjects/amsoftmax_face_recognition_caffe'

img_save_dir = os.path.join(root_dir, 'preprocess/result/CASIA_WebFace-%dx%d' % (img_resize[0], img_resize[1]))
dataset_dir = os.path.join(root_dir, 'preprocess/data/CASIA_WebFace/')
...
def CheckFace(face_boxes, index, img_w, img_h):
    box_x = face_boxes[index][0]
    box_y = face_boxes[index][1]
    width = face_boxes[index][2] - face_boxes[index][0]
    height = face_boxes[index][3] - face_boxes[index][1]
    if width < 30 or height < 30:
        return False
    centerx = box_x + width / 2
    centery = box_y + height / 2
    if centerx < img_w / 5 or centerx > img_w * 4 / 5 or centery < img_h / 5 or centery > img_h * 4 / 5:
        return False
    return True


def GetFaceIndex(face_boxes, key_points, img):

    face_num = face_boxes.shape[0]
    face_index = -1
    img_w = img.shape[1]
    img_h = img.shape[0]
    if face_num == 0:
        return face_index
    if face_num == 1:
        if not CheckFace(face_boxes, 0, img_w, img_h):
            return face_index
        face_index = 0
        return face_index
    if face_num > 1:
        ...
        for i in range(face_num):
            if CheckFace(face_boxes, i, img_w, img_h):
                index_all.append(i)
        if len(index_all) == 0:
            return -1
        if len(index_all) == 1:
        ...
        dist = []
        for i in index_all:
            box_x = face_boxes[i][0]
            box_y = face_boxes[i][1]
            width = face_boxes[i][2] - face_boxes[i][0]
            height = face_boxes[i][3] - face_boxes[i][1]
            centerx = box_x + width / 2
            centery = box_y + height / 2
            distance = math.pow(centerx - img_w / 2, 2) + math.pow(centery - img_h / 2, 2)
...
def Alignment(img, facial5points):
    ...
    crop_size = (img_resize[0] + of * 2, img_resize[1] + of * 2)
...
def main(show_img=False, face_alignment=False):
    root = '../tools/MTCNN/model/'

    minsize = 20

    caffe_model_path = '../tools/MTCNN/model/'
    ...
    image_list_txt = "%s.txt" % img_save_dir;

    if os.path.exists(image_list_txt):
        os.remove(image_list_txt)
    if os.path.exists('./result/null_img_list.txt'):
        os.remove('./result/null_img_list.txt')

    f_valid = open(image_list_txt, 'w')
    f_invalid = open('./result/null_img_list.txt', 'w')

    population = -1
    # error = []

    dir_list = os.listdir(dataset_dir)
    for img_id in dir_list:
        ...
        for file in os.listdir(dataset_dir + img_id):
            ...
            # check rgb position
            # tic()
            face_boxes, landmarks = detect_face(img_matlab, minsize, PNet, RNet, ONet, threshold, False, factor)
            # toc()

            num_face = face_boxes.shape[0]
            if num_face <= 0:
                f_invalid.write(dataset_dir + img_id + '/' + file + '\n')
                continue

            face_index = GetFaceIndex(face_boxes, landmarks, img_matlab)
            if face_index < 0:
                f_invalid.write(dataset_dir + img_id + '/' + file + '\n')
                continue
            else:
                if face_alignment == True:
                    # face_info = detect_info[face_index]
                    landmark = landmarks[face_index]
                    face_leye = np.array(landmark[0], landmark[1])
                    face_reye = np.array(landmark[2], landmark[3])
                    face_nose = np.array(landmark[4], landmark[5])
                    face_lmouth = np.array(landmark[6], landmark[7])
                    face_rmouth = np.array(landmark[8], landmark[9])
                    facial5points = np.array([face_leye, face_reye, face_nose, face_lmouth, face_rmouth])
                    face_resized = Alignment(img, facial5points)
                else:
                    left = math.ceil(face_boxes[face_index][0])
                ...
                # img_save_dir = './result/CASIA_WebFace' + '-' + str(img_resize[0]) + 'x' + str(img_resize[1])
                if not os.path.exists(os.path.join(img_save_dir, img_id)):
                    os.makedirs(os.path.join(img_save_dir, img_id))
                # face_gray = cv2.cvtColor(face_input, cv2.COLOR_BGR2GRAY)
                cv2.imwrite(os.path.join(img_save_dir, img_id) + '/' + file, face_resized)
                f_valid.write(os.path.join(img_save_dir, img_id) + '/' + file + ' ' + str(population) + '\n')

            if show_img:
                boxes = np.zeros((1, 5))
                boxes[0] = face_boxes[face_index]
                img = drawBoxes(img, boxes)
                cv2.imshow('img', img)
